...
export default function Chat() {
  ...
  const handleSubmit = async (e) => {
    ...
    //   const response = await chatCompletion(history); // 獲取 OpenAI 的回應
    //   const responseWithEmoji = `${response.content} ❤️`; // 在回應後附加愛心 Emoji
    
    //   // 更新消息列表
    //   setChatMessage((prev) => [
    //     ...prev,
    //     { content: responseWithEmoji, role: "assistant" },
    //   ]);
